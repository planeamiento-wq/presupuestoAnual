...
def generar_degrade_pastel_seguro(hex_base, cantidad=4):
    """
    Genera variaciones armónicas de degradé bajando el brillo/saturación 
    del color pastel inyectado, garantizando consistencia cromática.
    """
    try:
        ...
        for i in range(cantidad):
            ...
            if i == 1: new_r = max(0, new_r - 15)
            if i == 2: new_g = max(0, new_g - 15)
            if i == 3: new_b = max(0, new_b - 15)
                
            degrade.append(f"#{min(255, new_r):02x}{min(255, new_g):02x}{min(255, new_b):02x}")
        return degrade
    except Exception:
...
def renderizar_seccion_detalle_gastos(nombre_unidad, config_colores):
    """
    Componente agnóstico definitivo. Respeta la identidad pastel única 
    de la unidad seleccionada tanto en barras como en porciones circulares.
    """
    ...
    colores_pie_armonicos = generar_degrade_pastel_seguro(color_pastel_base, cantidad=4)

    # --- GRÁFICO 1: PERSONAL (CIRCULAR) ---
    with g_col1:
        st.markdown("<p style='font-weight: 700; color: #475569; text-align: center; margin-bottom: 15px;'>Distribución: Gastos de Personal</p>", unsafe_allow_html=True)
        
        labels_p = ["Docentes Ordinarios", "Docentes Interinos", "Autoridades", "Adscritos / No Doc."]
        ...
        fig_pie = go.Figure(go.Pie(
            ...
            values=valores_p,
            hole=0.45,
            marker=dict(colors=colores_pie_armonicos, line=dict(color='#ffffff', width=2)),
            textinfo="percent",
            textposition="inside",
        ...
        fig_pie.update_layout(
            ...
            legend=dict(orientation="h", yanchor="top", y=-0.05, xanchor="center", x=0.5, font=dict(size=11, color="#475569")),
    ...
    with g_col2:
        st.markdown("<p style='font-weight: 700; color: #475569; text-align: center; margin-bottom: 15px;'>Desglose: Gastos de Funcionamiento</p>", unsafe_allow_html=True)
        
        cat_f = ["Servicios Básicos", "Insumos Laboratorio", "Licencias / Software", "Mantenimiento", "Viáticos y Extensión"]
        val_f = [520, 380, 290, 150, 90]
